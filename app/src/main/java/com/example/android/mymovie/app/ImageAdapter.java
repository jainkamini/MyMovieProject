...
public class ImageAdapter extends ArrayAdapter<MovieItem> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // holder .imageView.setAdjustViewBounds(true);
      //  Picasso.with(context).load(item.get(position).).into(holder.imageView);
       // Picasso.with(context).load(item).into(holder.imageView);
       Picasso.with(context).load("http://image.tmdb.org/t/p/w185" + item.get(position).getmMoviePoster()).into(holder.imageView);
